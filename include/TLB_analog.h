...
float mes_VBAT() {
  ...
  return((analogReadMilliVolts(PIN_VBAT_MES) * ADCtoVoltScale) + ADC_VBAT_MES_OFFSET); // INEFFICIENT, but fancy ADC calibration (repeats calibration every time (i think))
}
...
const float _lithiumCellThresholds[4] = {3.3,3.5,4.2,4.4}; // (Volts) regular voltage thresholds for lithium cells
const float _lithiumCellThresholdTolerances[2] = {-0.05, 0.05}; // (Volts) tolerances on _lithiumCellThresholds
// const float _lithiumCellCalcLimit = 60.0f; // (Volts) don't bother counting cells beyond this voltage
const uint8_t _lithiumCellCalcLimit = 60.0f/_lithiumCellThresholds[2]; // (#ofCells) don't bother counting cells beyond this voltage
...
int8_t lithiumCellCalc(float VBAT_measurement) { // guesstimate how many (lithium) cells the battery has (to prevent over-discharging)
  ...
  else if(VBAT_measurement < (_lithiumCellThresholds[0]+_lithiumCellThresholdTolerances[0])) { return(0); } // don't even bother trying to count that low
  ...
  for(uint8_t i=1; i<=_lithiumCellCalcLimit; i++) {
    ...
    if((VBAT_measurement > (targets[1]+_lithiumCellThresholdTolerances[0])) && (VBAT_measurement < (targets[2]+_lithiumCellThresholdTolerances[1]))) {
      return(i); // nice and normal situation
    } else if((VBAT_measurement > (targets[0]+_lithiumCellThresholdTolerances[0])) && (VBAT_measurement < (targets[3]+_lithiumCellThresholdTolerances[1]))) { // extended battery voltage range
    ...
    } else if(VBAT_measurement < (targets[0]+_lithiumCellThresholdTolerances[0])) { // battery voltage lies between ranges, shit's ambiguous
